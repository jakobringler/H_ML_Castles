...
INPUT_SIZE_RNN = config.INPUT_SIZE_RNN
...
if config.HYPERPARAMETERSEARCH:
    ...
    HIDDEN_SIZES = [int(HIDDEN_SIZE_RNN/10), int(HIDDEN_SIZE_RNN), int(HIDDEN_SIZE_RNN*10)]
       
else: 
    BATCH_SIZES = [BATCH_SIZE]
    LEARNING_RATES = [LEARNING_RATE]
    HIDDEN_SIZES = [HIDDEN_SIZE_RNN]
...
path = os.path.join(hip, "../02_ml/02_parms/model/model_rnn.pth")
...
model = RNN(INPUT_SIZE_RNN, HIDDEN_SIZE_RNN, NUM_LAYERS, NUM_CLASSES).to(DEVICE)
        ...
        for hidden_size in HIDDEN_SIZES:
            ...
            for epoch in range(NUM_EPOCHS):
                for i, (input, target) in enumerate(train_loader):
                    ...
                    input = input.reshape(-1, SEQ_LENGTH, INPUT_SIZE_RNN).to(DEVICE)
